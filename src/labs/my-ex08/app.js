...
function setup(shaders) {
    ...
    connector = gl.getUniformLocation(program, "u_color");
...
function animate(time) {
    ...
    let xVal = Math.cos(time/1000) * 0.5;
    let yVal = Math.sin(time/1000) * 0.5;
    ...
    gl.uniform3f(connector, .0, .0, .0);

    gl.uniform4f(pos, xVal, yVal, 0.0, 0.0);

    gl.drawArrays(gl.TRIANGLES, 0, 3);

    gl.uniform3f(connector, 0.0, 1.0, 0.0);

    gl.uniform4f(pos, xVal, yVal, 0.0, 0.1);
